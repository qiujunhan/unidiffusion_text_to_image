...
class ClipCaptionModel(nn.Module):
    ...
    def __init__(self, prefix_length: int, hidden_dim=None, tokenizer_path="gpt2"):
        super(ClipCaptionModel, self).__init__()
        self.prefix_length = prefix_length
        eos = '<|EOS|>'
        special_tokens_dict = {'eos_token': eos}
        base_tokenizer = GPT2Tokenizer.from_pretrained(tokenizer_path)
        base_tokenizer.add_special_tokens(special_tokens_dict)
        self.gpt = GPT2LMHeadModel.from_pretrained(tokenizer_path, eos_token_id=base_tokenizer.eos_token_id)
...
class CaptionDecoder(object):
    def __init__(self, device, pretrained_path, hidden_dim=-1, tokenizer_path = "gpt2"):
        if hidden_dim < 0:
            hidden_dim = None
        ...
        self.caption_model = ClipCaptionModel(feature_length, hidden_dim=hidden_dim, tokenizer_path=tokenizer_path)
        # print("Load Model...")
