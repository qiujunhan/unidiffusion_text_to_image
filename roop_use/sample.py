...
def sample(prompt_index, config, nnet, clip_text_model, autoencoder, device,n=3,score_eval=None,chunk_size = 10):
    ...
    for idx in range(0, n):
        ...
        source_path = './resources/'+config.name+'_example.jpeg'
        target_path = sample_path
        output_path = 'temp/temp_images'
        read_img_pil(source_path)
        ...
        img_path = '-'.join([source_name,target_name])
        
        
        img_path = "temp/temp_images/"+img_path
        sample_img = read_img_pil(img_path)
